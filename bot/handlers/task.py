...
# Helper functions
async def _parse_task_id_from_callback(
    callback_data: str,
) -> tuple[bool, int | None, str]:
    """Parse task ID from callback data and validate it.

    :param callback_data: Callback data string
    :returns: Tuple of (success, task_id, error_message)
    """
    try:
        parts = callback_data.split("_")
        if len(parts) < 3 or not parts[-1].isdigit():
            return False, None, "❌ Error: invalid task data."

        task_id = int(parts[-1])
        if task_id < 0:
            return False, None, "❌ Error: invalid task ID."

        return True, task_id, ""
    except (ValueError, IndexError):
        return False, None, "❌ Error: could not parse task ID."


async def _handle_task_operation(
    ...
    """Generic handler for task operations (cancel, pause, resume).

    :param callback: Telegram callback query
    :param operation_name: Name of operation for logging
    :param operation_func: Async function to call for the operation
    :param success_emoji: Emoji for success message
    :param success_message_template: Template for success message (with {task_id})
    :param additional_info: Additional info to show in success message
    """
    ...
    try:
        # Parse task ID
        success, task_id, error_msg = await _parse_task_id_from_callback(callback.data)
        if not success:
            await callback.answer(error_msg)
            return

        logger.info(
            f"User {callback.from_user.id} attempting to {operation_name} task {task_id}"
        )

        user = await get_or_create_user(callback.from_user.id)
        logger.info(f"User {user.id} loaded for task {operation_name}")

        # Perform the operation
        operation_success = await operation_func(user.id, task_id)
        logger.info(
            f"Task {operation_name} result for task {task_id}: {operation_success}"
        )

        if operation_success:
            await callback.answer(
                f"{success_emoji} Task {operation_name}d successfully!"
            )

            success_text = success_message_template.format(task_id=task_id)
            if additional_info:
                success_text += f"\n{additional_info}"
            ...
            await callback.answer(f"❌ Could not {operation_name} task.")

    except Exception as e:
        logger.error(f"Error {operation_name}ing task: {e}", exc_info=True)
        await callback.answer(f"❌ Error {operation_name}ing task.")
...
async def handle_cancel_task(callback: CallbackQuery) -> None:
    ...
    await _handle_task_operation(
        callback=callback,
        operation_name="cancel",
        operation_func=cancel_user_task,
        success_emoji="✅",
        success_message_template="✅ Task #{task_id} has been cancelled successfully!",
    )
...
async def handle_pause_task(callback: CallbackQuery) -> None:
    ...
    await _handle_task_operation(
        ...
        operation_func=pause_user_task,
        success_emoji="⏸️",
        success_message_template="⏸️ Task #{task_id} has been paused successfully!",
        additional_info="Use /resume_task to resume the task.",
    )
...
async def handle_resume_task(callback: CallbackQuery) -> None:
    ...
    await _handle_task_operation(
        ...
        operation_func=resume_user_task,
        success_emoji="▶️",
        success_message_template="▶️ Task #{task_id} has been resumed successfully!",
    )
